...
int CDECL main(int argc, char **argv)
{
    ...
    std::wstring fullPath = converter.from_bytes(szFullExe[0] ? szFullExe : NULL);

    std::wstring fullCommand = converter.from_bytes(szCommand);

    std::wstring  fullDlls = converter.from_bytes(rpszDllsOut[0]);

    DWORD processId = 0;

    bool flag = true;

    NTSTATUS result = RhCreateAndInject(
        (wchar_t*)fullPath.c_str(),
        (wchar_t*)fullCommand.c_str(),
        CREATE_DEFAULT_ERROR_MODE | CREATE_SUSPENDED,
        EASYHOOK_INJECT_DEFAULT,
        (wchar_t*)fullDlls.c_str(),
        NULL,
        &flag,
        sizeof(bool),
        &processId
    ...
    if (FAILED(result))
	{
        ...
        printf("withdll.exe: RhCreateAndInject failed: %ld\n", dwError);
    ...
    if (fVerbose) {
        DumpProcess(processHandle);
    }

    WaitForSingleObject(processHandle, INFINITE);

    DWORD dwResult = 0;
    if (!GetExitCodeProcess(processHandle, &dwResult)) {
